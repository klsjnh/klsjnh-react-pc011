...
const systemMenuTree: MenuTreeNode[] = [
  ...
  { id: 3, title: '业务中心', path: '/business', icon: '💼', children: [
    ...
    { id: 36, title: '通知模板', path: '/business/template', icon: '✉️' },
    { id: 37, title: '消息推送', path: '/business/push', icon: '📣' },
    { id: 38, title: '数据统计', path: '/business/stats', icon: '📊' },
    { id: 39, title: '趋势分析', path: '/business/trend', icon: '📈' },
    { id: 40, title: '图表展示', path: '/business/charts', icon: '🎛' },
    { id: 41, title: '数据导出', path: '/business/export', icon: '📤' },
    { id: 42, title: '数据大屏', path: '/business/dashboard', icon: '🖥' },
  ]},
  { id: 4, title: '系统工具', path: '/tools', icon: '🛠', children: [
    ...
    { id: 46, title: '服务日志', path: '/business/servicelog', icon: '🔄' },
  ]},
];

function collectLeafPaths(nodes: MenuTreeNode[]): string[] {
  const paths: string[] = [];
  nodes.forEach(n => { if (n.children?.length) paths.push(...collectLeafPaths(n.children)); else paths.push(n.path); });
  return paths;
}
function collectAllPaths(nodes: MenuTreeNode[]): string[] {
  const paths: string[] = [];
  nodes.forEach(n => { paths.push(n.path); if (n.children) paths.push(...collectAllPaths(n.children)); });
  return paths;
}
          ...
          <div key={node.id}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '3px 0' }}>
              ...
              <input type="checkbox" checked={isChecked} ref={el => { if (el) el.indeterminate = isIndeterminate; }}
                onChange={(e) => { const all = hasChildren ? collectAllPaths([node]) : [node.path]; all.forEach(p => onCheck(p, e.target.checked)); }} />
              <span style={{ fontSize: '13px' }}>{node.icon} {node.title}</span>
              {hasChildren && <span style={{ fontSize: '10px', color: 'var(--text-muted)' }}>{checkedCount}/{leafPaths.length}</span>}
            </div>
            {hasChildren && isExpanded && <MenuCheckTree tree={node.children!} checkedPaths={checkedPaths} onCheck={onCheck} depth={depth + 1} />}
          </div>
        );
      })}
    </div>
...
export const PermissionsPage: React.FC = () => {
  ...
  const togglePermission = (key: string) => {
    setPermissionDraft(prev => { const n = new Set(prev); n.has(key) ? n.delete(key) : n.add(key); return n; });
  };
  ...
  const handleSaveEdit = () => {
    ...
    if (editModal.role) roleStore.updateRole(editModal.role.id, form);
    else roleStore.addRole({ ...form, status: 'active', permissions: ['/dashboard'], userIds: [] });
          ...
          <div className="modal-container" style={{ width: '320px' }} onClick={e => e.stopPropagation()}>
            <div className="modal-body" style={{ textAlign: 'center' }}>
              ...
              <div style={{ display: 'flex', gap: '10px', marginTop: '16px', justifyContent: 'center' }}>
                ...
                <button className="btn btn-danger" onClick={() => { roleStore.removeRole(dialog.id); setDialog({ visible: false, id: 0 }); }}>删除</button>
              </div>
            </div>
          </div>
        </div>
