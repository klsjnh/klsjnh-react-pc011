 ...
 * 默认数据模式走 vite proxy（/klsjnh 转发），baseOverride 固定为 /klsjnh/storage011，
...
/** storage011 模块前缀（请求 URL = STORAGE011_BASE + action） */
export const STORAGE011_BASE = '/klsjnh/storage011';

/**
 * 上传 / 下载走 multipart / blob，绕过 api.post 的 JSON 信封封装，需拼出完整基址。
 * 把 appConfig 里的 apiBaseUrl（默认 /klsjnh/system011）末段替换为 storage011，兼容完整 URL 配置。
 */
export function resolveStorageBase(): string {
  const base = appConfigStore.getSnapshot().apiBaseUrl || '/klsjnh/system011';
  return base.replace(/\/system011$/, '/storage011');
}
