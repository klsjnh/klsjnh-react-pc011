 ...
 * ⚠️ 2026-09-20 后端换版：ai011 改名 aicenter，前端 baseOverride 传 AICENTER_BASE（=/klsjnh/aicenter）。
 * 字段严格对齐后端 AiModelProviderController：主表 provider + 子表 api。
 * 分层：page → service → store；store 不调用 service。
 */
import { api } from '@/api/request';
import { julyAiModelProviderStore } from '@/stores/aiCenter/julyAiModelProviderStore';
import type {
  AiModelProviderItem,
...
export const AICENTER_BASE = '/klsjnh/aicenter';
...
export function selectProviderListByPage(body: object = {}): Promise<PageResult011<AiModelProviderItem>> {
  return api.post<PageResult011<AiModelProviderItem>>(AI_PROVIDER_ACTIONS.selectListByPage, body, AICENTER_BASE);
}
