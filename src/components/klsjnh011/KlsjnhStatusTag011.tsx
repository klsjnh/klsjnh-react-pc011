 ...
 *
 * 内置启用/停用默认映射（'1' 启用 green / 其他 停用 red），数字 0/1 与字符串 '0'/'1' 都认；
...
export interface KlsjnhStatusTag011Props {
  ...
  /** 值 → antd Tag color 映射（默认 { '1': 'green', '0': 'red' }） */
  colors?: Record<string, string>;
  /** 完全接管：原始值 → 文案（未命中回落 '?' 不渲染裸值） */
...
export function KlsjnhStatusTag011({ value, labels, colors, values }: KlsjnhStatusTag011Props) {
  ...
  const text = values?.[key] ?? labels?.[key] ?? (key === '1' ? '启用' : key === '0' ? '停用' : '');
  const color = colors?.[key] ?? (key === '1' ? 'green' : 'red');
